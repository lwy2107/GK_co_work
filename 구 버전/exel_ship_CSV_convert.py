...
def process_file(file_path):
    ...
    for row in range(1, 1000):  # 1부터 6까지의 행
        for col in range(1, 18):  # 1부터 14까지의 열 (A부터 Q까지)
    ...
    columns_to_clear = [4, 5, 7, 8]  # E, F, H, I열에 해당하는 열 번호
    for col in columns_to_clear:
        for cell in sheet.iter_cols(min_col=col, max_col=col):
            for row in cell:
                row.value = None  # 값 삭제

    for source, destination in zip(sheet.iter_cols(min_col=6, max_col=6, min_row=1), sheet.iter_cols(min_col=4, max_col=4, min_row=1)):
    ...
    # J~N열의 내용을 F~J열로 이동
    for source_col, destination_col in zip(range(9, 18), range(5, 14)):
...
def convert_excel_to_csv(file_path):
    ...
    print(f"변환된 CSV 파일: {csv_file_path}")
